...
{
	int numLodVertices = vvd->numLodVertices[0];
	mVertices = new Format::VVD::Vertex[numLodVertices];
	if(vvd->numFixups == 0) {
		for(int v=0; v<numLodVertices; v++) {
			mVertices[v] = *vvd->vertex(v);
		}
	} else {
		int v = 0;
		for(int i=0; i<vvd->numFixups; i++) {
			...
			for(int j=0; j<fixup->numVertices; j++) {
				mVertices[v] = *vvd->vertex(fixup->sourceVertexId + j);
				v++;
			}
		}
	}
